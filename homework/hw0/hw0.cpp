...
int main() {
	...
	*/
	...
	// ---------------------------  question 2-c -------------------------------
	// part i
	robot_q << 0.0, 0.5, -90.0 * (M_PI / 180.0); // modify this
	robot->setQ(robot_q);
	...
	// part ii
	robot_q << 90.0  * (M_PI / 180.0), 0.5, -90.0 * (M_PI / 180.0); // modify this
	robot->setQ(robot_q);
	robot->updateKinematics();
	ee_position = robot->position(ee_link_name, ee_pos_in_link);
	...
	robot->setQ(robot_q);
	robot->updateModel();
	//file_2e_i << (robot->M())(0,1) << "\t" << (robot->M())(1,1) << "\t" << (robot->M())(2,1) << "\n"; // modify this
	int n_steps = 250;
	for(int i=0 ; i < n_steps ; i++)
	{
		// write your code
		robot_q << 0.0, 0.5, (-90.0 + i*180.0/n_steps) * (M_PI / 180.0); // modify this
		robot->setQ(robot_q);
		robot->updateModel();
		file_2e_i << (robot->M())(0,1) << "\t" << (robot->M())(1,1) << "\t" << (robot->M())(2,1) << "\n"; 
	...
	robot->setQ(robot_q);
	robot->updateModel();
	//file_2e_ii << (robot->M())(0,1) << "\t" << (robot->M())(1,1) << "\t" << (robot->M())(2,1) << "\n"; // modify this
	n_steps = 250;
	for(int i=0 ; i < n_steps ; i++)
	{
		robot_q << 0.0, i*2.0/n_steps, 0.0; // modify this
		robot->setQ(robot_q);
		robot->updateModel();
		file_2e_ii << (robot->M())(0,1) << "\t" << (robot->M())(1,1) << "\t" << (robot->M())(2,1) << "\n"; 
	}
	...
	// write your code
	ofstream file_2g;
	file_2g.open("../../homework/hw0/q2-g.txt");
	robot_q << 0.0, 0.5, 45.0* (M_PI / 180.0), 0.0;
	robot->setQ(robot_q);
	robot->updateModel();
	g = robot->jointGravityVector();
	//file_2f_ii << g.transpose() << "\n";
	...
	{
		// write your code
		robot_q << 0.0, 0.5, 45.0* (M_PI / 180.0), i*1.0/n_steps; // modify this
		robot->setQ(robot_q);
		robot->updateModel();
		g = robot->jointGravityVector();
		file_2g << g.transpose() << "\n";
